...
function [w_opt, history, verbose] = adam_optimizer_DG(grad_f, w0, max_iter, alpha, beta1, beta2, epsilon, tolerance, beta_h)
...
arguments
        ...
        beta_h    double = 1e-8   % hyper gradient step size (needs tuning)
end
...
%ADAM Hyperparameter descent
u_prev = zeros(size(w0));   % Adam update direction u_{t-1}, Baydin (2018)
...
while (norm(gradient) > tolerance) && (t < max_iter)
    ...
    % Update the learning rate (alpha) using the Hyperparameter descent
    h_alpha = -gradient'*u_prev;
    alpha = alpha - beta_h * h_alpha;
    alpha   = max(alpha, 0);            % safeguard against negative learning rates
    ...
    u = m_hat./(sqrt(v_hat) + epsilon); % Current Descent direction
    w = w - alpha * u;
    u_prev = u; %save for next loop iteration

    %record the history of parameter values
    history.w(:,t) = w;
    
    %record the history of learning rates (alpha)
    history.alpha(t) = alpha;
