...
%For the AMSGrad upgrade
v_hat_max = zeros(size(w0));
...
while (norm(gradient) > tolerance) && (t < max_iter)
    ...
    % (AMSGrad addition)
    v_hat_max = max(v_hat_max, v_hat);   %  keep the max of the previous and current v_hat
    w = w - alpha * m_hat ./ (sqrt(v_hat_max) + epsilon);  % use max
